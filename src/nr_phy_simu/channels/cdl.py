...
class CdlChannel(FadingChannelBase):
    ...
    def _generate_path_coefficients(
        ...
        normalized_delays = torch.as_tensor([cluster.normalized_delay for cluster in profile.clusters], dtype=REAL_DTYPE)
        default_power_db = torch.as_tensor([cluster.power_db for cluster in profile.clusters], dtype=REAL_DTYPE)
        ...
        if delays_s.numel() != len(profile.clusters):
        ...
        for cluster_idx, cluster in enumerate(profile.clusters):
            cluster_matrix = torch.zeros((num_rx_ant, num_tx_ant, num_samples), dtype=COMPLEX_DTYPE)
            for ray_offset in RAY_OFFSETS:
                ...
                ray_doppler = float(torch.dot(arrival_vec, velocity_vec).item()) / wavelength

                time = torch.arange(num_samples, dtype=REAL_DTYPE) / sample_rate_hz
                phase = float((2 * math.pi * torch.rand(1, generator=self.rng, dtype=REAL_DTYPE)).item())
                ray_process = torch.exp(1j * (2 * math.pi * ray_doppler * time + phase))
                rx_response = self._array_response(num_rx_ant, float(arrival_vec[0].item()), rx_spacing)
                tx_response = self._array_response(num_tx_ant, float(departure_vec[0].item()), tx_spacing)
                cluster_matrix += (
                    rx_response[:, None, None]
                    * torch.conj(tx_response)[None, :, None]
                    * ray_process[None, None, :]
                )

            cluster_matrix /= math.sqrt(len(RAY_OFFSETS))
            if cluster.fading.upper() == "LOS":
                k_factor_db = float(config.channel.params.get("k_factor_db", CDL_LOS_K_DB.get(profile_name, 0.0)))
                los_arrival = self._unit_vector(cluster.aoa_deg, cluster.zoa_deg)
                los_departure = self._unit_vector(cluster.aod_deg, cluster.zod_deg)
                los_process = self._rician_process(
                    ...
                    specular_doppler_hz=float(torch.dot(los_arrival, velocity_vec).item()) / wavelength,
                )
                rx_response = self._array_response(num_rx_ant, float(los_arrival[0].item()), rx_spacing)
                tx_response = self._array_response(num_tx_ant, float(los_departure[0].item()), tx_spacing)
                cluster_matrix = (
                    rx_response[:, None, None]
                    * torch.conj(tx_response)[None, :, None]
                    * los_process[None, None, :]
                )

            coeff[:, :, cluster_idx, :] = math.sqrt(float(path_powers[cluster_idx].item())) * cluster_matrix

        return delays_s, coeff

    @staticmethod
    def _unit_vector(azimuth_deg: float, zenith_deg: float) -> torch.Tensor:
        az = math.radians(azimuth_deg)
        ze = math.radians(zenith_deg)
        sin_ze = math.sin(ze)
        return torch.tensor(
            [
                sin_ze * math.cos(az),
